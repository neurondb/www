#!/usr/bin/env python3
"""
Fix arrow connections in all tutorial SVG files using regex.
This approach is more robust for files with XML issues.
Arrows must:
...
def parse_box_from_rect(rect_match) -> Optional[Tuple[float, float, float, float, float, float]]:
    """Parse rect attributes and return (x, y, width, height, tx, ty) where tx,ty is transform offset."""
    x = float(rect_match.group('x') or 0)
    y = float(rect_match.group('y') or 0)
    width = float(rect_match.group('width') or 0)
    height = float(rect_match.group('height') or 0)
    
    # Check for transform in parent group
    # This is a simplified approach - we'll need to track transforms separately
    return (x, y, width, height, 0, 0)

def extract_boxes_and_transforms(content: str) -> List[Tuple[float, float, float, float, float, float]]:
    """Extract all boxes with their transforms."""
    boxes = []
    
    # Find all groups with transforms
    group_pattern = r'<g\s+transform="translate\(([\d.]+),\s*([\d.]+)\)"[^>]*>.*?</g>'
    
    # Find rects in groups
    group_rect_pattern = r'<g\s+transform="translate\(([\d.]+),\s*([\d.]+)\)"[^>]*>.*?<rect\s+([^>]+)>'
    
    for match in re.finditer(group_rect_pattern, content, re.DOTALL):
        tx = float(match.group(1))
        ty = float(match.group(2))
        rect_attrs = match.group(3)
        ...
        if width > 0 and height > 0:
            boxes.append((x, y, width, height, tx, ty))
    
    # Also find rects not in groups
    ...
    for match in re.finditer(rect_pattern, content):
        ...
        if width > 0 and height > 0:
            boxes.append((x, y, width, height, 0, 0))
    
    return boxes

def get_box_center_bottom(box: Tuple[float, float, float, float, float, float]) -> Tuple[float, float]:
    """Get center-bottom point of a box."""
    x, y, width, height, _, _ = box
    return (x + width / 2, y + height)

def get_box_center_top(box: Tuple[float, float, float, float, float, float]) -> Tuple[float, float]:
    """Get center-top point of a box."""
    x, y, width, height, _, _ = box
    return (x + width / 2, y)

def get_box_center(box: Tuple[float, float, float, float, float, float]) -> Tuple[float, float]:
...
def fix_svg_arrows(svg_path: Path):
    ...
    try:
        ...
        # Extract boxes
        boxes = extract_boxes_and_transforms(content)
        if len(boxes) < 2:
            return False
        
        # Find all path elements with marker-end - use a more careful pattern
        ...
        def replace_path(match):
            ...
            # Update attributes carefully, preserving marker-end
            new_attrs = attrs
            # Replace d attribute
